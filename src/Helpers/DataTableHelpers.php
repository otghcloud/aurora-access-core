...
namespace OTGH\AccessControl\Core\Helpers;
...
class DataTableHelpers
{
    ...
    public static function actionsDropdown(array $actions, string $buttonLabel = '<i class="fa-solid fa-fw fa-gear"></i>'): string
    {
        ...
        foreach ($actions as $action) {
            ...
            $iconHtml = $iconClass !== ''
                ? '<i class="'.self::escape($iconClass).'" aria-hidden="true"></i> '
                : '';

            $itemsHtml .= '<a '.$attrString.'>'.$iconHtml.self::escape($label).'</a>';
        }

        return '<button class="btn btn-sm dropdown-toggle align-text-top" data-bs-boundary="viewport" data-bs-toggle="dropdown" aria-expanded="false">'.self::escapeButtonLabel($buttonLabel).'</button>'
    ...
    private static function escapeButtonLabel(string $buttonLabel): string
    {
        return $buttonLabel === '<i class="fa-solid fa-fw fa-gear"></i>'
            ? $buttonLabel
